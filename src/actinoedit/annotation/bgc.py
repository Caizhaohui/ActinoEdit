...
def load_bgc_regions(path: str | Path) -> list[BGCRegion]:
    ...
    - Minimal antiSMASH-like (future: parse region GBK features).
    ...
    with open(p) as f:
        for line in f:
            ...
            try:
                ...
                # Skip bad line but keep going (robust for user data)
                continue
...
def find_nearest_bgc(
    ...
    Returns:
        (BGCRegion or None, distance)
        Distance == 0 if inside a BGC.
    """
...
def annotate_guides_with_bgc(
    ...
    """Return a new list of guides annotated with BGC context.

    For each guide, sets:
      - bgc_id / bgc_type if the guide spacer is inside a BGC
      - bgc_context describing inside or nearest cluster + distance

    This function does not mutate the input guides (uses replace).

    Args:
        guides: Original guide candidates.
        bgc_regions: List of known BGCs.
        near_threshold: How far (bp) to consider a guide "near" a BGC.

    Returns:
        List of (possibly annotated) GuideCandidate.
    """
    annotated: list[GuideCandidate] = []

    for guide in guides:
        # Use cut_site or middle of spacer as representative position
        ...
        if nearest:
            ...
            annotated.append(guide)  # unchanged
